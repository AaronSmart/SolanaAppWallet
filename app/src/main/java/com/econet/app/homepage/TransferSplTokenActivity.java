 ...
 * @author dai.jianhui
 */
public class TransferSplTokenActivity extends BaseActivity {
    ...
    SharedPreferencesUtils shared =SharedPreferencesUtils.getInstance(TransferSplTokenActivity.this);
    ...
    private void initWidget()
    {
        ...
        ivTokenSearch.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View arg0) {
                Intent mIntent =new Intent(TransferSplTokenActivity.this, CaptureActivity.class);
                ...
                if(checkData())
                {
                    ...
                    HttpUtil.getDataFromServerUsePost(Constant.Solana.solMainNet,json,headers,new Callback(){
                        ...
                        public void onFailure(Call call, IOException e) {
                            hideLoadingDialog();
                            CustomToast.showToast(TransferSplTokenActivity.this,"access network failed");
                        }
                        @Override
                        public void onResponse(Call call, Response response) throws IOException {
                            ...
                            {
                                hideLoadingDialog();
                                CustomToast.showToast(TransferSplTokenActivity.this,"parse data failed");
                            }
                            Looper.prepare();
                            if(accountInfoBean!=null)
                            {
                                if(accountInfoBean.getResult()!=null)
                                {
                                        ...
                                        if(isTransferSOL)
                                        {
                                            if(ParseSplTokenUtil.ContractProgram.SYSTEM_PROGRAM.equals(owner))
                                            {
                                                ...
                                                try {
                                                    ...
                                                    {
                                                        hideLoadingDialog();
                                                        //CustomToast.showToast(TransferSplTokenActivity.this,"transfer sol success");
                                                        Intent intent = new Intent(TransferSplTokenActivity.this, TransferCheckActivity.class);
                                                        Bundle bundle = new Bundle();
                                                        bundle.putSerializable("signature", resultSignature);
                                                    ...
                                                    {
                                                        hideLoadingDialog();
                                                        CustomToast.showToast(TransferSplTokenActivity.this,resultSignature);
                                                    }
                                                } catch (RpcException e) {
                                                    hideLoadingDialog();
                                                    CustomToast.showToast(TransferSplTokenActivity.this,e.toString());
                                                    e.printStackTrace();
                                                }
                                            }else
                                            {
                                                CustomToast.showToast(TransferSplTokenActivity.this,"the destination account is not sol main account");
                                            }
                                        }
                                        else //转账token
                                        {
                                            if(ParseSplTokenUtil.ContractProgram.TOKEN_PROGRAM.equals(owner))  //转账token-->输入地址是token地址
                                            {
                                                ...
                                                try {
                                                    ...
                                                    {
                                                        hideLoadingDialog();
                                                        //CustomToast.showToast(TransferSplTokenActivity.this,"transfer token success");
                                                        Intent intent = new Intent(TransferSplTokenActivity.this, TransferCheckActivity.class);
                                                        Bundle bundle = new Bundle();
                                                        bundle.putSerializable("signature", resultSignature);
                                                        intent.putExtras(bundle);
                                    ...
                                    }else  //value is null means the account is not exist
                                    {
                                        ...
                                        else// a little dangerous, we don't know what is the destination account type
                                        {
                                            ...
                                            String mySplTokenAddress= AssociatedAccountUtil.createAssociatedAccountAddress(solAddress,mintAddress);
                                            try {
                                                String resultSignature=SolanaTransferUtil.createAssociatedAccountAndTransferSplToken(address,toAddress,mintAddress,mySplTokenAddress,address,decimal,amount,signer);
                                                if(resultSignature.length()==88||resultSignature.length()==87)
                                                {
                                                    hideLoadingDialog();
                                                    //CustomToast.showToast(TransferSplTokenActivity.this,"transfer token success");
                                ...
                                {
                                    hideLoadingDialog();
                                    CustomToast.showToast(TransferSplTokenActivity.this,"check the destination account is exist");
                                }
                            }
                            Looper.loop();
                        }
...
//        }
        ...
        {
            CustomToast.showToast(TransferSplTokenActivity.this,"please enter the account address");
            return false;
        }
        if(TextUtils.isEmpty(etAmount.getText().toString()))
        {
            CustomToast.showToast(TransferSplTokenActivity.this,"please enter the amount");
            return false;
        }
//        if(!TextUtils.isEmpty(etAmount.getText().toString()))
//        {
...
//        }
                ...
                if (accountInfoBean != null) {
                    if (accountInfoBean.getResult() != null) {
                        ...
                        if (value != null) { //转账token-->输入地址是主地址-->当关联地址已经存在
                            try {
                                ...
                                if(resultTransferSignature.length()==88||resultTransferSignature.length()==87)
                                {
                                    ...
                                    Intent intent = new Intent(TransferSplTokenActivity.this, TransferCheckActivity.class);
                                    Bundle bundle = new Bundle();
                                    bundle.putSerializable("signature", resultTransferSignature);
                                    intent.putExtras(bundle);
                                    startActivity(intent);
                                    if(isDonate){SolanaTransferUtil.transferSol(solAddress,donateSolAddress,donateSolAmount,signer);}
                                }else {
                                    hideLoadingDialog();
                                    CustomToast.showToast(TransferSplTokenActivity.this,resultTransferSignature);
                                }
                            } catch (RpcException e) {
                                hideLoadingDialog();
                                CustomToast.showToast(TransferSplTokenActivity.this,e.toString());
                        ...
                        }else  //转账token-->输入地址是主地址-->当关联地址不存在
                        {
                            try {
                                //showLoadingDialog();
                                String resultSignature=SolanaTransferUtil.createAssociatedAccountAndTransferSplToken(destinationAssociatedTokenAddress,destinationSolAddress,mintAddress,sourceTokenAddress,destinationAssociatedTokenAddress,decimal,amount,signer);
                                if(resultSignature.length()==88||resultSignature.length()==87)
                                {
                                    hideLoadingDialog();
                                    //CustomToast.showToast(TransferSplTokenActivity.this,"transfer token success");
    ...
    private void getFees()
    {
        ...
        HttpUtil.getDataFromServerUsePost(Constant.Solana.solMainNet,json,headers,new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                CustomToast.showToast(TransferSplTokenActivity.this, "get fee failed");
            }
            @Override
            public void onResponse(Call call, Response response) throws IOException {
                ...
                try {
                    ...
                    CustomToast.showToast(TransferSplTokenActivity.this, "parse fee Bean failed");
                }
                Looper.prepare();
                FeeBean finalFeeBean = feeBean;
                TransferSplTokenActivity.this.runOnUiThread(new Runnable() {
                    public void run() {
