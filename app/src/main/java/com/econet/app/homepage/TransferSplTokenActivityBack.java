 ...
 * @author dai.jianhui
 */
public class TransferSplTokenActivityBack extends BaseActivity {
    ...
    SharedPreferencesUtils shared =SharedPreferencesUtils.getInstance(TransferSplTokenActivityBack.this);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ButterKnife.bind(TransferSplTokenActivityBack.this);
        setTitle("Transfer");
    ...
    private void initWidget()
    {
        ...
        ivTokenSearch.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View arg0) {
                Intent mIntent =new Intent(TransferSplTokenActivityBack.this, CaptureActivity.class);
                ...
                if(checkData())
                {
                    ...
                    HttpUtil.getDataFromServerUsePost(Constant.Solana.solMainNet,json,headers,new Callback(){
                        ...
                        public void onFailure(Call call, IOException e) {
                            hideLoadingDialog();
                            CustomToast.showToast(TransferSplTokenActivityBack.this,"access network failed");
                        }
                        @Override
                        public void onResponse(Call call, Response response) throws IOException {
                            ...
                            {
                                hideLoadingDialog();
                                CustomToast.showToast(TransferSplTokenActivityBack.this,"parse data failed");
                            }
                            Looper.prepare();
                            if(accountInfoBean!=null)
                            {
                                        ...
                                        if(isTransferSOL)
                                        {
                                            if(ParseSplTokenUtil.ContractProgram.SYSTEM_PROGRAM.equals(owner))
                                            {
                                                ...
                                                try {
                                                    ...
                                                    {
                                                        hideLoadingDialog();
                                                        CustomToast.showToast(TransferSplTokenActivityBack.this,"transfer sol success");
                                                        if(isDonate){SolanaTransferUtil.transferSol(solAddress,donateSolAddress,donateSolAmount,signer);}
                                                    }else
                                                    {
                                                        hideLoadingDialog();
                                                        CustomToast.showToast(TransferSplTokenActivityBack.this,resultSignature);
                                                    }
                                                } catch (RpcException e) {
                                                    hideLoadingDialog();
                                                    CustomToast.showToast(TransferSplTokenActivityBack.this,e.toString());
                                                    e.printStackTrace();
                                                }
                                            }else
                                            {
                                                CustomToast.showToast(TransferSplTokenActivityBack.this,"the destination account is not sol main account");
                                            }
                                        }
                                        else //转账token
                                        {
                                            if(ParseSplTokenUtil.ContractProgram.TOKEN_PROGRAM.equals(owner))  //转账token-->输入地址是token地址
                                            {
                                                ...
                                                try {
                                                    ...
                                                    {
                                                        hideLoadingDialog();
                                                        CustomToast.showToast(TransferSplTokenActivityBack.this,"transfer token success");
                                                        if(isDonate){SolanaTransferUtil.transferSol(solAddress,donateSolAddress,donateSolAmount,signer);}
                                                    }else
                                                    {
                                                        hideLoadingDialog();
                                        ...
                                        else// a little dangerous, we don't know what is the destination account type
                                        {
                                            ...
                                            try {
                                                ...
                                                if(resultSignature.length()==88||resultSignature.length()==87)
                                                {
                                                    ...
                                                    {
                                                        hideLoadingDialog();
                                                        CustomToast.showToast(TransferSplTokenActivityBack.this,"transfer success");
                                                        if(isDonate){SolanaTransferUtil.transferSol(solAddress,donateSolAddress,donateSolAmount,signer);}
                                                    }else
                                                    {
                                                        hideLoadingDialog();
                                                        CustomToast.showToast(TransferSplTokenActivityBack.this,resultTransferSignature);
                                                    }
                                                }else {
                                                    hideLoadingDialog();
                                                    CustomToast.showToast(TransferSplTokenActivityBack.this,resultSignature);
                                ...
                                {
                                    hideLoadingDialog();
                                    CustomToast.showToast(TransferSplTokenActivityBack.this,"check the destination account is exist");
                                }
                            }
                            Looper.loop();
                        }
...
//        {
//            CustomToast.showToast(TransferSplTokenActivityBack.this,"please select the document type");
//            return false;
//        }
        if(TextUtils.isEmpty(etAccountAddress.getText().toString()))
        {
            CustomToast.showToast(TransferSplTokenActivityBack.this,"please enter the account address");
            return false;
        }
        if(TextUtils.isEmpty(etAmount.getText().toString()))
        {
            CustomToast.showToast(TransferSplTokenActivityBack.this,"please enter the amount");
            return false;
        }
//        if(!TextUtils.isEmpty(etAmount.getText().toString()))
//        {
...
//        }
    ...
    private void getFees()
    {
        ...
        HttpUtil.getDataFromServerUsePost(Constant.Solana.solMainNet,json,headers,new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                CustomToast.showToast(TransferSplTokenActivityBack.this, "get fee failed");
            }
            @Override
            public void onResponse(Call call, Response response) throws IOException {
                ...
                try {
                    ...
                    CustomToast.showToast(TransferSplTokenActivityBack.this, "parse fee Bean failed");
                }
                Looper.prepare();
                FeeBean finalFeeBean = feeBean;
                TransferSplTokenActivityBack.this.runOnUiThread(new Runnable() {
                    public void run() {
